...
#Human Models, Control vs. overweight + obesity - 2nd categorization, (Balanced)
#frequency <= 40 Hz and >= 0.3 Hz, time window for each observation (size.bin) = 60000 ms,
#Proportion 80/20, Maximum reading of files (size.of.register) = 360000 ms, with Tuning.

library(caret)
remove.columns <- c("patient", "patient.number","group", "obs")

m2.photo.60s.t <- read.csv("DataH_and_A/wavelet2022/dfControlvsSobrepesoyObesidad2daCat_0.3_40HzBalanceado.csv")
...
m2.model.control.humans.plot <- SingleModelROCPlot(m2.model.control.humans$performance, "Modelo SANO vs Sobrepeso + Obesidad  0.3 - 40Hz")
