...
# Human model Mesotopic DL, frecuency <= 40 Hz and >= 0.3 Hz,
...
#m2.foto.60s <- ERGBatchProcess("DataH_and_A/espcondobscuridadBalanceadosExacto/",
#                               max.lecture = 900, size.bin = 60000,
#                               remove.registers = 1:5, filtrar.altas = T,
#                               size.of.register = 300000)
m2.foto.60s <- read.csv("DataH_and_A/fourier2021/HumansMesotopicBalanced.csv")
...
#Plots ROC
m2.model.sano.sindrome.met.plot <- SingleModelROCPlot(m2.model.sano.sindrome.met$performance, "Model Health vs Disorder Mesotopic")
